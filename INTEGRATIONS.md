...
**codegraph** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `codegraph` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
codegraph ... --format json | cognis-connect emit --to brief   # analyst brief via your /v1 fleet
codegraph ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `codegraph` outputs:

```python
from cognis_connect import normalize, edgemesh
findings = [normalize(rec, source="codegraph") for rec in records]   # records = your JSON output
