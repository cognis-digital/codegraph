...
How **codegraph** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `codegraph` lives in the **Private-AI backbone** cluster.
...
| from | relation | to |
|---|---|---|
| `codegraph` | JSON findings compose with | [`edgemesh`](https://github.com/cognis-digital/edgemesh), [`modelroute`](https://github.com/cognis-digital/modelroute), [`uncensored-fleet`](https://github.com/cognis-digital/uncensored-fleet), [`cognis-code`](https://github.com/cognis-digital/cognis-code) |
| `codegraph` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `codegraph` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `codegraph` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the private-ai backbone cluster.** `codegraph` output feeds the next tool:
```bash
codegraph ... --format json > out.json          # this tool's findings (see `codegraph --help`)
...
codegraph ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
codegraph ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
codegraph ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `codegraph`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `codegraph` slots
